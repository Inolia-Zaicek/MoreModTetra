...
public class ModularWhiteBag extends ModularItem implements ICurioItem { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    //部件类型/槽位——[slot]
    //法杖核心————决定法杖类型≈配重部位
    public final static String mmt_white_bag_white = "mmt_white_bag/white";
    public final static String mmt_white_bag_red = "mmt_white_bag/red";
    public final static String mmt_white_bag_orange = "mmt_white_bag/orange";
    public final static String mmt_white_bag_yellow = "mmt_white_bag/yellow";
    public final static String mmt_white_bag_green = "mmt_white_bag/green";
    public final static String mmt_white_bag_cyan = "mmt_white_bag/cyan";
    public final static String mmt_white_bag_blue = "mmt_white_bag/blue";
    public final static String mmt_white_bag_purple = "mmt_white_bag/purple";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_mmt_white_bag";
    ...
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_mmt_white_bag”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_mmt_white_bag"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_mmt_white_bag”已正确注册。*/
    //modular_mmt_white_bag算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_mmt_white_bag"
    )
    public static ModularWhiteBag instance;
    /*** ModularNecklace 类的构造函数。* 这里初始化了该模块化物品的基本属性。*/
    public ModularWhiteBag() {
        ...
        canHone = false;
        //设置主要部件有什么
        majorModuleKeys = new String[]{mmt_white_bag_white,mmt_white_bag_red, mmt_white_bag_orange,mmt_white_bag_yellow,
                mmt_white_bag_green,mmt_white_bag_cyan,mmt_white_bag_blue,mmt_white_bag_purple};
        //设置次要部件有什么
        minorModuleKeys = new String[]{};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{mmt_white_bag_white};
    }
