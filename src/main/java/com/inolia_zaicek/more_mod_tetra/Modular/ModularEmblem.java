...
public class ModularEmblem extends ModularItem implements  ICurioItem { // 声明一个名为Modularring的公共类，它继承自ModularItem并实现ICurio接口。
    @Override
    public boolean canEquip(SlotContext slotContext, ItemStack stack) {
        return MMTUtil.noSameCurio(slotContext.entity(), MoreModTetraItemRegister.MODULAR_Emblem.get());
    }
    //部件类型/槽位——[slot]
    public final static String mmt_emblemEmblem_type = "mmt_emblem/emblem_type";
    public final static String mmt_emblemEmblem_ring = "mmt_emblem/emblem_ring";
    //public final static String mmt_emblemAttachment = "mmt_emblem/attachment";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_mmt_emblem";
    ...
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_mmt_emblem”物品实例。
     * 这允许其他模组或代码直接通过`Modularring.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_mmt_emblem"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_mmt_emblem”已正确注册。*/
    //modular_mmt_emblem算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_mmt_emblem"
    )
    ...
    public ModularEmblem() {
        ...
        canHone = false;
        //设置主要部件有什么
        majorModuleKeys = new String[]{mmt_emblemEmblem_type, mmt_emblemEmblem_ring};
        //设置次要部件有什么
        minorModuleKeys = new String[]{};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{mmt_emblemEmblem_type,mmt_emblemEmblem_ring};
    }
