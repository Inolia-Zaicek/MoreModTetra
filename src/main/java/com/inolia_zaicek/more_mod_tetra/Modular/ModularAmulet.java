...
public class ModularAmulet extends ModularItem implements  ICurioItem { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    //部件类型/槽位——[slot]
    public final static String mmt_amuletChain = "mmt_amulet/envelope";
    public final static String mmt_amuletPendant = "mmt_amulet/fill";
    public final static String mmt_amuletDye = "mmt_amulet/dye";
    //public final static String mmt_amuletAttachment = "mmt_amulet/attachment";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_mmt_amulet";
    ...
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_mmt_amulet”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_mmt_amulet"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_mmt_amulet”已正确注册。*/
    //modular_mmt_amulet算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_mmt_amulet"
    )
    public static ModularAmulet instance;
    ...
    public ModularAmulet() {
        ...
        canHone = false;
        //设置主要部件有什么
        majorModuleKeys = new String[]{mmt_amuletChain, mmt_amuletPendant};
        //设置次要部件有什么
        minorModuleKeys = new String[]{mmt_amuletDye};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{mmt_amuletChain, mmt_amuletPendant};
    }
    ...
    public boolean canEquip(SlotContext slotContext, ItemStack stack) {
        return MMTUtil.noSameCurio(slotContext.entity(), MoreModTetraItemRegister.MODULAR_Amulet.get());
    }
}