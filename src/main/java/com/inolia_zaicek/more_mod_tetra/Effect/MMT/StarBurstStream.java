...
public class StarBurstStream {
    public static final InteractionHand INTERACTION_HAND = InteractionHand.OFF_HAND;

    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(starBurstStreamEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                starBurstStreamName, 0, 18, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(starBurstStreamTooltip, statGetter)
        );
            ...
            ItemStack offHandItem = player.getOffhandItem();
                ...
                if (mainHandItem.getItem() instanceof ItemModularHandheld item&&mainHandItem.getItem() instanceof ItemModularHandheld item2) {
                    ...
                    if (mainEffectLevel > 0&&offEffectLevel>0&&MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
                        //上buff部分
                        if (!player.hasEffect(MMTEffectsRegister.StarBurstStream.get())) {
                            player.addEffect(new MobEffectInstance(MMTEffectsRegister.StarBurstStream.get(), 150, 0));
                        } else {
                            int buffLevel = player.getEffect(MMTEffectsRegister.StarBurstStream.get()).getAmplifier();
                            player.addEffect(new MobEffectInstance(MMTEffectsRegister.StarBurstStream.get(), 150, Math.min(17, buffLevel + 1)));
                            //根据星爆气流斩缩减无敌帧
