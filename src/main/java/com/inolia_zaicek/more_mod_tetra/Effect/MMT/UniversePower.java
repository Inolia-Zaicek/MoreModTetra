...
public class UniversePower {
    ...
    private static final String UNIVERSE_POWER_TIME_NBT = MoreModTetra.MODID + ":universe_power_time";
    // NBT 键的前缀，用于标记不同伤害类型的增伤。
    private static final String DAMAGE_TYPE_TAG_PREFIX = "more_mod_tetra_damage_type";
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (event.getSource().getEntity() instanceof LivingEntity livingEntity) {
            // 获取玩家的增伤等级和最大种类等级
            float damageLevel = MMTEffectHelper.getInstance().getMainOffHandSumEffectLevel(livingEntity, universe_power_Effect);
            int numberLevel = (int) MMTEffectHelper.getInstance().getMainOffHandSumEffectEfficiency(livingEntity, universe_power_Effect);
