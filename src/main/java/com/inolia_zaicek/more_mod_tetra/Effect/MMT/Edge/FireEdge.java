...
public class FireEdge {
    ...
    public static void init() {
        var statGetter = new StatGetterEffectLevel(fireEdgeEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                fireEdgeName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(fireEdgeTooltip, statGetter)
        );
        ...
        if (MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
            ...
            if (event.getSource().getEntity() instanceof LivingEntity player) {
                ...
                if (mainHandItem.getItem() instanceof IModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, fireEdgeEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel += (int) mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof IModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, fireEdgeEffect);
                    if (offEffectLevel > 0) {
                ...
                if (effectLevel > 0) {
                    ...
                    var DamageType = MMTTickZero.hasSource(player.level(), DamageTypes.ON_FIRE, player);
