        ...
        if (MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
            ...
            if (event.getSource().getEntity() instanceof LivingEntity player) {
                ...
                if (mainHandItem.getItem() instanceof IModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, dragonBreathEdgeEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel += (int) mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof IModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, dragonBreathEdgeEffect);
                    if (offEffectLevel > 0) {
                ...
                if (effectLevel > 0) {
                    ...
                    var DamageType = MMTTickZero.hasSource(player.level(), DamageTypes.DRAGON_BREATH, player);
