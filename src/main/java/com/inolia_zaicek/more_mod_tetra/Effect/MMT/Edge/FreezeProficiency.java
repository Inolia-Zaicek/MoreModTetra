...
public class FreezeProficiency {
    ...
    public static void init() {
        var statGetter = new StatGetterEffectLevel(freezeProficiencyEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                freezeProficiencyName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(freezeProficiencyTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (event.getSource().getDirectEntity() instanceof Player player) {
                ...
                if (effectLevel > 0&&event.getSource()==mob.damageSources().freeze()) {
