...
public class FreezeCombo {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(freezeComboEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                freezeComboName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(freezeComboTooltip, statGetter)
        );
        ...
        if (MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
            ...
            if (event.getSource().getEntity() instanceof LivingEntity player) {
                ...
                if (mainHandItem.getItem() instanceof IModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, freezeComboEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel += (int) mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof IModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, freezeComboEffect);
                    if (offEffectLevel > 0) {
