...
public class WitherProficiency {
    ...
    public static void init() {
        var statGetter = new StatGetterEffectLevel(witherProficiencyEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                witherProficiencyName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(witherProficiencyTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (event.getSource().getDirectEntity() instanceof LivingEntity player) {
                ...
                if (mainHandItem.getItem() instanceof IModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, witherProficiencyEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel += (int) mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof IModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, witherProficiencyEffect);
                    if (offEffectLevel > 0) {
