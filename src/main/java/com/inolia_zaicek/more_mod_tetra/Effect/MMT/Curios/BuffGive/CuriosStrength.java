...
public class CuriosStrength {
    ...
    public static void tick(LivingEvent.LivingTickEvent event) {
        ...
        if (ModList.get().isLoaded("curios")) {
            float effectLevel = MMTCuriosHelper.getInstance().getCuriosEffectLevel(player, curiosStrengthEffect);
            if (effectLevel > 0 && player.level().getGameTime() % 20L == 0) {
                player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST,300,0));
            }
        }
        int effectLevel2 = MMTEffectHelper.getInstance().getMainOffHandMaxEffectLevel(player,strength_buff_Effect);
        if (effectLevel2 > 0 && player.level().getGameTime() % 10L == 0) {
            player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST,100,effectLevel2-1));
        }
    }
}