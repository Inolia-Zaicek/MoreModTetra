...
public class ExceedSlash {
    ...
    public static void init() {
        ...
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                exceedSlashName, 0, 1, false, false, false,
    ...
    public static void hurt(LivingHurtEvent event) {
            ...
            if (event.getSource().getDirectEntity() instanceof Player player) {
                ...
                if (mainHandItem.getItem() instanceof ModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, exceedSlashEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel +=  mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof ModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, exceedSlashEffect);
                    if (offEffectLevel > 0) {
                ...
                if (effectLevel > 0&&hp<mhp) {
                    //比例*增伤==如100-99/100==1%%已损失*1%增伤
                    float finish =1+dhp*(effectLevel/100);
