...
public class GrenadeEffectTraitHurt {
    ...
    public static void hurt(LivingHurtEvent event) {
        if (ModList.get().isLoaded("l2complements")) {
            //挨打的是玩家且攻击者非空
            if (event.getEntity() instanceof Player player &&event.getSource().is(DamageTypeTags.IS_EXPLOSION)) {
            ...
            //攻击者是玩家
            if (event.getSource().getDirectEntity() instanceof Player player &&event.getEntity()!=null&&event.getSource().is(DamageTypeTags.IS_EXPLOSION)) {
                CuriosApi.getCuriosInventory(player).ifPresent(inv -> inv.findCurios
                                (itemStack -> itemStack.getItem() instanceof ModularItem).forEach(
                                slotResult -> {
                                    slotResult.stack();
