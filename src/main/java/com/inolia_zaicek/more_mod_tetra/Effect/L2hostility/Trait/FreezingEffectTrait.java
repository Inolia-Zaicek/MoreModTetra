            ...
            if (event.getSource().getDirectEntity() instanceof Player player &&event.getEntity()!=null) {
                CuriosApi.getCuriosInventory(player).ifPresent(inv -> inv.findCurios
                                ...
                                slotResult -> {
                                    ...
                                    int effectLevel = curiousItem.getEffectLevel(itemStack, freezingEffectTraitEffect);
                                    //获取一下玩家主副手
                                    ItemStack mainHandItem = player.getMainHandItem();
                                    ItemStack offhandItem = player.getOffhandItem();
                                    if (mainHandItem.getItem() instanceof ModularItem item) {
                                        float mainEffectLevel = item.getEffectLevel(mainHandItem, freezingEffectTraitEffect);
                                        if (mainEffectLevel > 0) {
                                            effectLevel += (int) mainEffectLevel;
                                        }
                                    }
                                    if (offhandItem.getItem() instanceof ModularItem item) {
                                        float offEffectLevel = item.getEffectLevel(offhandItem, freezingEffectTraitEffect);
                                        if (offEffectLevel > 0) {
                                    ...
                                    if (effectLevel > 0&&event.getEntity()!=null) {
                                        ...
                                        map.put(Objects.requireNonNull(ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("l2complements", "frozen")))
                                                ,new MobEffectInstance(Objects.requireNonNull(ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("l2complements", "frozen"))), 200*effectLevel, effectLevel - 1));
                                        mob.addEffect(new MobEffectInstance(Objects.requireNonNull(ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("l2complements", "frozen"))), 200*effectLevel, effectLevel - 1));
                                    }
                                }
                        )
                );
