...
public class BloodStaffSocket {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(bloodStaffSocketEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                bloodStaffSocketName, 0, 10, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(bloodStaffSocketTooltip, statGetter)
        );
        ...
        if (event.getSource().getEntity() instanceof LivingEntity attacker) {
            if(event.getSource().is(ISSDamageTypes.BLOOD_MAGIC)) {
                ...
                if (mainStack.getItem() instanceof IModularItem item) {
                    float level = item.getEffectLevel(mainStack, bloodStaffSocketEffect);
                    if (level > 0) {
                        event.setAmount(event.getAmount()*1.1f);
                    }
                } else if (offStack.getItem() instanceof IModularItem item) {
                    float level = item.getEffectLevel(offStack, bloodStaffSocketEffect);
                    if (level > 0) {
                        event.setAmount(event.getAmount()*1.1f);
                    }
                }
