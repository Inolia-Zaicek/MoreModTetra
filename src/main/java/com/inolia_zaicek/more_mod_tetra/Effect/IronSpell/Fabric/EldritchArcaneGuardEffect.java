...
public class EldritchArcaneGuardEffect {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(eldritchArcaneGuardEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                eldritchArcaneGuardName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(eldritchArcaneGuardTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (mainHandItem.getItem() instanceof IModularItem item) {
            float level = item.getEffectLevel(mainHandItem, eldritchArcaneGuardEffect);
            if (level > 0) {
                //获取法强属性
            ...
            float level = item.getEffectLevel(offhandItem, eldritchArcaneGuardEffect);
            if (level > 0) {
                //获取法强属性
                float magic = (float) target.getAttributeValue(AttributeRegistry.ELDRITCH_MAGIC_RESIST.get());
                float resist = (float) target.getAttributeValue(AttributeRegistry.SPELL_RESIST.get());
