...
public class EvocationArcaneEdgeEffect {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(evocationArcaneEdgeEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                evocationArcaneEdgeName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(evocationArcaneEdgeTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (attackingEntity instanceof LivingEntity attacker) {
            ...
            if (heldStack.getItem() instanceof IModularItem item) {
                ...
                //获取词条数值
                float level = MMTEffectHelper.getInstance().getMainHandEffectLevel(attacker, evocationArcaneEdgeEffect);
                // 只有当 "Esoteric Edge" 效果等级大于 0 时才执行以下操作。
                if (level > 0&& MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
                    //基础攻击伤害量
                    float baseAmount = event.getAmount();
                    //获取法强属性
                    float fire = (float) (attacker.getAttributeValue(AttributeRegistry.EVOCATION_SPELL_POWER.get())-1)*(level/100)+1;
                    ...
                    //获取伤害类型
                    var FIRE_MAGIC_DAMAGE = MMTTickZero.hasSource(attacker.level(), ISSDamageTypes.EVOCATION_MAGIC,attacker);
