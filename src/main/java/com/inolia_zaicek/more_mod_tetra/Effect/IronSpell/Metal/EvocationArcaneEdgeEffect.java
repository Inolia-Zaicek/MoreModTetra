    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (attackingEntity instanceof LivingEntity attacker) {
            ...
            if (heldStack.getItem() instanceof IModularItem item) {
                ...
                //获取词条数值
                float level = item.getEffectLevel(heldStack, evocationArcaneEdgeEffect);
                ...
                if (level > 0&& MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
                    ...
                    //获取法强属性
                    float fire = (float) attacker.getAttributeValue(AttributeRegistry.EVOCATION_SPELL_POWER.get());
