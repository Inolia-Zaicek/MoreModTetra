...
public class ProtectionArcaneEdgeEffect {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(protectionArcaneEdgeEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                protectionArcaneEdgeName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(protectionArcaneEdgeTooltip, statGetter)
        );
        ...
        if (attackingEntity instanceof LivingEntity attacker) {
            ...
            if (heldStack.getItem() instanceof IModularItem item) {
                ...
                if (level > 0&& MMTDamageSourceHelper.isMeleeAttack(event.getSource())) {
                    ...
                    float b = (float) attacker.getAttributeValue(AttributeRegistry.ICE_MAGIC_RESIST.get());
                    float c = (float) attacker.getAttributeValue(AttributeRegistry.LIGHTNING_MAGIC_RESIST.get());
                    float d = (float) attacker.getAttributeValue(AttributeRegistry.EVOCATION_MAGIC_RESIST.get());
                    float e = (float) attacker.getAttributeValue(AttributeRegistry.HOLY_MAGIC_RESIST.get());
                    float f = (float) attacker.getAttributeValue(AttributeRegistry.BLOOD_MAGIC_RESIST.get());
                    float g = (float) attacker.getAttributeValue(AttributeRegistry.ENDER_MAGIC_RESIST.get());
                    float h = (float) attacker.getAttributeValue(AttributeRegistry.NATURE_MAGIC_RESIST.get());
                    float i = (float) attacker.getAttributeValue(AttributeRegistry.ELDRITCH_MAGIC_RESIST.get());
