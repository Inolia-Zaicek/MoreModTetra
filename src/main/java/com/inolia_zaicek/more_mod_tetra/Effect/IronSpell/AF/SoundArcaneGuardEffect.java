...
public class SoundArcaneGuardEffect {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(soundArcaneGuardEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                soundArcaneGuardName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(soundArcaneGuardTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (mainHandItem.getItem() instanceof ModularItem item) {
            float level = item.getEffectLevel(mainHandItem, soundArcaneGuardEffect);
            if (level > 0) {
                //获取法强属性
            ...
            float level = item.getEffectLevel(offhandItem, soundArcaneGuardEffect);
            if (level > 0) {
                //获取法强属性
                float magic = (float) target.getAttributeValue(net.alshanex.alshanex_familiars.registry.AttributeRegistry.SOUND_MAGIC_RESIST.get());
                float resist = (float) target.getAttributeValue(AttributeRegistry.SPELL_RESIST.get());
