...
public class FireArcaneGuardEffect {
    @OnlyIn(Dist.CLIENT)
    public static void init() {
        var statGetter = new StatGetterEffectLevel(fireArcaneGuardEffect, 1);
        GuiStatBar statBar = new GuiStatBar(0, 0, StatsHelper.barLength,
                fireArcaneGuardName, 0, 100, false, false, false,
                statGetter, LabelGetterBasic.integerLabel,
                new TooltipGetterInteger(fireArcaneGuardTooltip, statGetter)
        );
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        float baseAmount = event.getAmount();
            ...
            if (mainHandItem.getItem() instanceof IModularItem item) {
                float level = item.getEffectLevel(mainHandItem, fireArcaneGuardEffect);
                if (level > 0) {
                    //获取法强属性
                    float magic = (float) target.getAttributeValue(AttributeRegistry.FIRE_MAGIC_RESIST.get());
                ...
                float level = item.getEffectLevel(offhandItem, fireArcaneGuardEffect);
                if (level > 0) {
                    //获取法强属性
                    float magic = (float) target.getAttributeValue(AttributeRegistry.FIRE_MAGIC_RESIST.get());
                    float resist = (float) target.getAttributeValue(AttributeRegistry.SPELL_RESIST.get());
