        ...
        if(ModList.get().isLoaded("eidolon")) {
            if (event.getSource().getEntity() instanceof LivingEntity livingEntity) {
                ...
                if (mainHandItem.getItem() instanceof IModularItem item) {
                    float mainEffectLevel = item.getEffectLevel(mainHandItem, shadowGemEffect);
                    if (mainEffectLevel > 0) {
                        effectLevel +=  mainEffectLevel;
                    }
                }
                if (offhandItem.getItem() instanceof IModularItem item) {
                    float offEffectLevel = item.getEffectLevel(offhandItem, shadowGemEffect);
                    if (offEffectLevel > 0) {
                        effectLevel += (int) offEffectLevel;
                    }
                }
                if (effectLevel > 0) {
                    event.getEntity().addEffect(new MobEffectInstance(EidolonPotions.UNDEATH_EFFECT.get(), effectLevel*20, 0, true, true, true));
                }
            }
        }
    }
