package com.inolia_zaicek.more_mod_tetra.ModularCurios.Iron; // 定义该类所属的包，表示它是“More Mod Tetra”模组中“Modular Curios”部分的一部分。
...
public class ModularIronStaff extends ItemModularHandheld { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    ...
    //法杖手柄
    public final static String mmt_iron_staff_handle = "mmt_iron_staff/handle";
    //柄末水晶（镶嵌
    public final static String mmt_iron_staff_crystals = "mmt_iron_staff/crystals";
    //杖体铭刻（覆层
    public final static String mmt_iron_staff_inscription = "mmt_iron_staff/inscription";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_mmt_iron_staff";
    ...
    private static final GuiModuleOffsets majorOffsets = new GuiModuleOffsets( -20, 9, 4, 0, 4, 18, -4, 0);
    //根据部件数量变化
    //-12,-1左上  -21,12左中  -12,25左下
    private static final GuiModuleOffsets minorOffsets = new GuiModuleOffsets(-12, -3, -12, 27);
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_mmt_iron_staff”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_mmt_iron_staff"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_mmt_iron_staff”已正确注册。*/
    //modular_mmt_iron_staff算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_mmt_iron_staff"
    )
    public static ModularIronStaff instance;
    /*** ModularNecklace 类的构造函数。* 这里初始化了该模块化物品的基本属性。*/
    public ModularIronStaff() {
        ...
        canHone = true;
        //设置主要部件有什么
        majorModuleKeys = new String[]{mmt_iron_staff_core,mmt_iron_staff_head,mmt_iron_staff_handle};
        //设置次要部件有什么
