...
public class MMTEffectHelper {
    public static MMTEffectHelper INSTANCE;
    public static MMTEffectHelper getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new MMTEffectHelper();
        }
        return INSTANCE;
    }
    //单独获取主副手
    ...
    public static void trackPastPositions(ArrayList<MMTEffectHelper.PastPosition> pastPositions, Vec3 currentPosition, float distanceThreshold) {
        for(MMTEffectHelper.PastPosition pastPosition : pastPositions) {
            ++pastPosition.time;
        }

        if (!pastPositions.isEmpty()) {
            MMTEffectHelper.PastPosition latest = (MMTEffectHelper.PastPosition)pastPositions.get(pastPositions.size() - 1);
            float distance = (float)latest.position.distanceTo(currentPosition);
            if (distance > distanceThreshold) {
                pastPositions.add(new MMTEffectHelper.PastPosition(currentPosition, 0));
            }
        } else {
            pastPositions.add(new MMTEffectHelper.PastPosition(currentPosition, 0));
        }
