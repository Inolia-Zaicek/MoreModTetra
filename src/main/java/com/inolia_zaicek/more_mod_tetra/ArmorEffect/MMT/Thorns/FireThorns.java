...
public class FireThorns {
    ...
    public static void hurt(EffectLevelEvent event) {
        if (event.hurtEvent.getSource().getEntity() instanceof LivingEntity mob) {
            ...
            float effectLevel = MMTEffectHelper.getInstance().getAllArmorSumEffectLevel(livingEntity, fire_thorns_Effect);
            if(effectLevel>0) {
                var DamageType = MMTTickZero.hasSource(livingEntity.level(), DamageTypes.LAVA, livingEntity);
