...
public class ArmorLastAndFinalStand {
    ...
    public static void hurt(EffectLevelEvent event) {
        ...
        if (event.hurtEvent.getSource().getEntity() instanceof LivingEntity livingEntity) {
            ...
            if (effectLevelN > 0 && hp > 0) {
                event.addNormalMulti(( (finalDhp) * (effectLevelN / 100)));
            }
        }
    }
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (event.getEntity() != null) {
            ...
            //最高减伤
            float effectLevel = MMTEffectHelper.getInstance().getAllArmorSumEffectLevel(livingEntity, armor_last_stand_Effect);
            //需求血量比例
            float effectEfficiency = MMTEffectHelper.getInstance().getAllArmorSumEffectEfficiency(livingEntity, armor_last_stand_Effect);
            float mhp = livingEntity.getMaxHealth();
            float hp = livingEntity.getHealth();
            //最大生命值-当前生命值=已损失生命值 /最大生命值=已损失比例
            float dhp = (mhp - hp) / mhp;
            //反向阈值
            float dEffect = 1 - effectEfficiency/100;
            //例：阈值30%，当前55%——dhp=45/100，阈值为30/100，需要dhp=70/100，应取dhp-反向阈值）/反向阈值——即阈值进度
            //如果阈值40，当前40%，也就是dhp=0.6 dEffect = 0.6，完美
            float finalDhp =Math.min(1, ( dhp-dEffect ) / ( dEffect ) );
            if (effectLevel > 0 && hp > 0) {
                event.setAmount(event.getAmount()*(1 - (finalDhp) * (effectLevel / 100)));
            }
        }
    }
}